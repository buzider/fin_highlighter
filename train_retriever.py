...
def quick_generate_contrastive_data(
    ...
    for idx, training_element in enumerate(training_annotated_results):
        ...
        # tokenized_stringA + SEP
        sep_token_id = tokenizer.sep_token_id
        sep_token_tensor = torch.tensor([sep_token_id], device=device)
        tokenized_stringA_with_sep = torch.cat([tokenized_stringA, sep_token_tensor])
        seq_len_A_with_sep = tokenized_stringA_with_sep.size(0)
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(selected_texts), batch_size):
                ...
                tokenized_stringB = batch_tokenized['input_ids'].to(device)  # (batch_size, seq_len_B)

                batch_size_actual = tokenized_stringB.size(0)

                
                expanded_A = tokenized_stringA_with_sep.unsqueeze(0).expand(batch_size_actual, -1)
                combined_input_ids_padded = torch.cat([expanded_A, tokenized_stringB], dim=1)
                ...
                probs = torch.sigmoid(logits_before_sep)[..., 1]  # (batch_size, seq_len_A)
